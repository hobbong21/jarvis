...
| 진화 관측 | **Telemetry & Feedback** | ✅ | `telemetry.log_turn()` — 턴별 메타(백엔드/폴백/지연/intent/TTS결과) JSONL 저장. `GET /api/harness/telemetry` 로 집계 조회. PII(본문) 미수집. **사이클 #3 #3**: `web/harness/dashboard.html` SPA — 토큰 입력(Bearer 헤더 only) + 5초 자동 새로고침 + Evolve 버튼. **사이클 #4 #1**: handle_audio(음성) 경로도 `log_turn()` 기록, summary 에 `input_channels` / `tts_regen_count` / `tts_regen_rate` 추가, 메인 페이지 우상단 ⚙ 제어판 버튼이 새 탭 대시보드 오픈. **사이클 #4 #2**: `telemetry.subscribe/_notify` pub-sub 모델 + `WS /api/harness/ws` — 새 턴 발생 시 즉시 push, 25초 keepalive, QueueFull 시 사일런트 drop. **사이클 #5 #1**: `summarize().latency` 추가 — fanout/llm/tts/total_ms 의 nearest-rank `avg/p50/p95/p99/count`. 대시보드 "응답시간 분포" 표 노출. respond_internal/compare 도 total_ms 기록. |
| 진화 자동 제안 | **Self-Evolution** | ✅ | **사이클 #3 #5**: `harness_evolve.propose_next_cycle()` + `POST /api/harness/evolve` — 누적 텔레메트리 (≥`MIN_TURNS`) 를 LLM 에 보내 차세대 사이클 markdown 초안 자동 생성, `harness/sarvis/proposals/cycle-{n}.md` 저장. min_turns 외부 입력은 상향만 허용. **사이클 #5 #3**: `harness_evolve.export_proposal_to_github()` + `POST /api/harness/evolve/export` — 생성된 proposal 을 GitHub Issue 로 자동 게시. PROPOSALS_DIR 안 .md 만 허용 (path traversal 차단, symlink 우회도 `Path.resolve()` + `relative_to()` 로 차단), repo 는 인자 또는 env(`HARNESS_GITHUB_REPO`/`GITHUB_REPO`), token 은 env(`GITHUB_TOKEN`/`GH_TOKEN`) **만** (body 미수용 — 누출 방지), `issue_url` https://github.com/ allowlist. **GitHub Enterprise(사설 호스트) 미지원 — 의도된 정책** (api.github.com 하드코딩, 토큰/링크 노출면 최소화). 대시보드 evolve 결과에 "GitHub Issue 로 내보내기" 버튼 + dry-run 옵션. |
| 회귀 안전망 | **Regression Tests** | ✅ | **사이클 #5 #2**: `tests/test_telemetry.py` (12) + `tests/test_evolve_export.py` (14, symlink 우회/허용 2건 포함) — 총 26 PASS. unittest 기반 (의존성 추가 X). summarize 빈/비빈 키셋 동등성, PII 차단, 백분위 정확성, pub-sub 격리, GitHub export traversal/symlink 우회/누락 토큰/body 잘림 등 검증. `python -m unittest discover tests` 로 일괄 실행. |
