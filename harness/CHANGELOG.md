...
- **`commands/evolve.md` 슬래시 커맨드** — `/harness:evolve "<피드백 인용>"` 진입점 신설. SKILL.md Phase 7(하네스 진화)을 단일 커맨드로 호출 가능하게 함. EN/KO/JA 3국어 본문 + 실행 에이전트용 구현 노트 포함. (이전까지 README는 `/harness:evolve`를 언급했으나 실 명령 파일이 없었음 — gap 해소)
- **`docs/show-hn-launch-kit.md`** (255줄) — 2026-05-06 Wed 07:05 PT Show HN 런칭 플레이북 복원. Title A/B/C, ~380단어 body, T-72h~T+72h 타임라인, post-launch 분기, 5%-oversold 대응, crossposting 룰 포함. (post-M0 audit §1 C-3에 PASS 기록되었으나 디스크에 부재했던 자산 복구)
- **`_workspace/community/issue-2-reply.md`** (63줄) — hesreallyhim 직접 인용("really good stuff ... Nice job.") + Layer/Sub-layer 뱃지 + "Harness Factories" 카테고리 제안 포함 공식 응답 초안 복원. (post-M0 audit D-8 PASS 자산)
- **`_workspace/community/issue-3-reply.md`** (84줄) — Gemini PoC 로드맵 P-01 + SaehwanPark/meta-harness Codex 포트 + Gizele1/harness-init·OpenRig 인접 대안 명시한 다중 런타임 질문 응답 초안 복원. (post-M0 audit D-7 PASS 자산)

### Changed

- **`skills/harness/SKILL.md` Phase 7 진입점 박스 추가** — `/harness:evolve` 슬래시 커맨드를 Phase 7의 단일 진입점으로 명시. 자동 트리거(7-4 조건) 외에도 수동 명령으로 진화 메커니즘 호출 경로 확보.
- **`skills/harness/SKILL.md` 산출물 체크리스트 명확화** — `.claude/commands/` 항목을 "사용자 프로젝트에는 생성하지 않음. harness 플러그인 자신의 commands/는 별개로 메타 명령 제공"으로 정정. 이전 표현은 플러그인 자체 commands/까지 금지하는 것으로 오독 가능했음.
- **`docs/show-hn-launch-kit.md` 요일 표기 정정** — T-0/T+24h/T+48h/T+72h 4건의 weekday 표기를 실제 캘린더(2026-05-06=Wed, 5-7=Thu, 5-8=Fri, 5-9=Sat)에 맞춰 정정.

### Fixed

- **`_workspace/release/audit-2026-04-18.md` §3.3 정합성** — 원안 "plugin.json은 건드리지 않음"이 post-M0 audit §2 #1과 모순됨을 해결. Errata 노트 추가하여 표제를 "version 필드 무변경"으로 한정 해석. description/keywords 변경은 CHANGELOG [1.2.1] Changed 블록에 이미 정상 기록되어 위생 위반 아님을 명시.
