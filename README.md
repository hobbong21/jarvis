...
### Harness Agent (HA) — 자체 진화 (사이클 #29)

하네스 자기 자신도 같은 안전 게이트로 개선되는 closed-loop 시스템.

| 에이전트 | 역할 |
|---------|------|
| **Observer** | SARVIS 트레이스 → Issue Card |
| **Diagnostician** | 5 Whys + 가설 → Diagnosis |
| **Strategist** | 8 카테고리 + 강제 Do-Nothing → Strategy |
| **Improver** | PatchSpec (텍스트 명세, 실제 파일 수정 절대 없음) |
| **Validator** | 위험 점수 + `auto_approval_blocked=True` 고정 |
| **Reporter** | 사용자 대시보드 노출 |
| **MetaEvaluator** ★ | **하네스 자기 자신**의 퍼널/효과성/신뢰도를 측정 → 자기-Issue 발화 |

자체 진화 루프:
```
MetaEvaluator → 자기-Issue ("채택률 낮음" 등) → 동일 파이프라인
    → ha_proposals(pending) → 사람 승인 → ha_outcomes 추적 → 다음 사이클 학습
```

**자율 적용은 절대 없음** — 모든 변경은 사람 승인 큐에 누적되며, 적용 결과는
`ha_outcomes` 테이블에 resolved/persistent/regressed 로 라벨링되어 다음 사이클의
MetaEvaluator 평가에 반영됨 (closed-loop).
