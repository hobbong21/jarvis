...
## 0c. 사이클 #25 — HA Stage S3 (Strategist + Improver + Validator, L1) 요약

- **신규 모듈 3종**: `sarvis/ha/strategist.py` (8 카테고리, Do Nothing
  강제), `improver.py` (Strategy→PatchSpec, 텍스트 명세만), `validator.py`
  (target prefix 위험 + reversibility + 코드경로 + diag_conf, risk_level
  low/med/high, `auto_approval_blocked=True` 고정).
- **메모리**: `ha_strategies`/`ha_proposals`/`ha_validations` 신규 3 테이블
  + append-only 트리거 (strategies/validations). proposals 는 status
  갱신 필요로 트리거 제외(주석 명시). 메서드 11종.
- **서버 WS 5종**: `ha_run_strategist`/`ha_run_improver`/
  `ha_run_validator`/`ha_proposals_list`/`ha_proposal_decision`. 인증
  게이트 + Kill Switch 게이트 동일 적용. 승인되어도 적용은 일어나지 않으며
  응답에 `applied=False, note="L1 — Stage S4 도입 전"` 명시.
- **UI**: 4 버튼(Strategist/Improver/Validator/제안 큐) + 위험 배지 +
  승인·거부 인라인 버튼.
- **신규 테스트 22건**: S3 단위 17 (Strategist 6 + Improver 5 + Validator 4
  + Memory guard 5) + WS round-trip 5. **회귀 696/696 통과** (이전 674
  → +22).
- **안전**: Strategist/Improver/Validator 모두 `_FORBIDDEN_WRITE` 7종 거부
  + DB 트리거 (strategies/validations append-only) + Validator 의 자동
  승인 영구 차단(L1).
- **Do Nothing 강제** (기획서 §4.3.3): 모든 진단당 마지막 strategy 가
  반드시 `do_nothing` — 자율 시스템의 행동 편향 차단.
...
- **활성 에이전트** (2/8): Observer, Reporter (미니).
- **자율 등급**: L0 (Observe-only) — 어떠한 코드/안전 프롬프트 변경도 발생 안 함.
- **신규 모듈**: `sarvis/ha/{__init__,base,safety,observer,reporter}.py`
  (메시지 HMAC 서명, 7원칙 가드, 5종 휴리스틱 anomaly, One-Pager 보고서).
- **메모리 4 테이블**: `ha_messages`(append-only), `ha_issues`,
  `ha_kill_switch_log`, `ha_optout`. 옵트아웃 사용자는 LEFT JOIN NULL 로
  Observer 입력에서 자동 제외.
- **WS 5종**: `ha_run_observer`, `ha_issues_list`, `ha_kill_switch`,
  `ha_optout`, `ha_growth_diary`. 인증 게이트 + Kill Switch 전역 거부.
- **UI**: prod-dock "성장 일기 (HA · Stage S1)" 카드 — Observer 실행, 윈도우
  선택, 옵트아웃/Kill Switch 토글, 이슈 카드 N개.
- **신규 테스트 22건**: HA 단위 17 (HMAC, 메시지 검증, 5 휴리스틱, 옵트아웃
  격리, Kill Switch 안전측 ON, append-only) + WS 통합 5 (5종 round-trip
  + Kill Switch 차단). **회귀 657/657 통과** (이전 635 → +22).
- **위협 모델**: 자기 코드 수정/사용자 데이터 송출/안전 프롬프트 변경/감사
  로그 위변조/Kill Switch 우회/공모/타 에이전트 영향 — 모두 코드·권한
  분리로 차단(`HAAgent._FORBIDDEN_WRITE` 7종).
- **로드맵**: S2 Diagnostician(L1) → S3 Strategist+Improver+Validator(L2 PR
  생성) → S4 MetaEvaluator(L3 staged-rollout). 상세는
  `docs/SARVIS_HA_AGENT_MATRIX.md`.
...
| 자동 테스트 총 건수 | **657** (사이클 #22 시점 635 → +22 HA) |
| 테스트 통과율 | **100%** |
| 사이클 #22 신규 코드 | `memory.py` +180 LOC (feedback/summary) + `server.py` +60 LOC + UI +180 LOC |
| 사이클 #22 신규 테스트 | 25 건 (test_feedback 19 + WS 6) |
| 사이클 #23 신규 코드 | `sarvis/ha/` 5 파일 + `memory.py` +260 LOC + `server.py` +130 LOC + UI +110 LOC |
| 사이클 #23 신규 테스트 | **22 건** (HA 단위 17 + WS 통합 5) |
